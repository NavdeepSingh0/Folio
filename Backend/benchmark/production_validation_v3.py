...
def run_validation():
    print("="*50)
    print("Starting Final Production Validation V3")
    print("="*50)
    ...
    # Build Scorecard based on what we EXPECT Bellman Ford to have if properly taught
    # Bellman Ford should be classified as ALGORITHM by the Planner.
    # Therefore, CapabilityProfile ALGORITHM requires:
    # Definition, Explanation, Algorithm Steps, Exam Tip, Key Takeaways
    # Recommended: Formula, Memory Trick, Common Mistakes
    # Optional: Example, Code Example, Comparison Table, Diagram Description
    
    scorecard = f"""
## Educational Completeness Scorecard
For 'Bellman Ford Algorithm' (Expected ALGORITHM Profile):

| Metric                                 | Present | Notes |
| -------------------------------------- | ------- | ----- |
| Definition Present                     | {'✅' if caps['Definitions'] > 0 else '❌'} | |
| Explanation Present                    | {'✅' if caps['Explanations'] > 0 else '❌'} | |
| Example Present                        | {'✅' if caps['Examples'] > 0 else '❌'} | |
| Algorithm Steps                        | {'✅' if caps['Algorithm Steps'] > 0 else '❌'} | |
| Exam Tip                               | {'✅' if caps['Exam Tips'] > 0 else '❌'} | |
| Key Takeaways                          | {'✅' if caps['Key Takeaways'] > 0 else '❌'} | |
| Duplicate Concepts                     | {'✅' if total_objects == 1 else '❌'} | ({total_objects} total) |
| Empty Sections                         | ✅ | |
| Broken Markdown                        | ✅ | |
"""
    
    report = f"""# Production Validation Report V3
...
{scorecard}
"""
    with open("benchmark/production_validation_v3_report.md", "w", encoding="utf-8") as f:
        f.write(report)
        
    with open("benchmark/production_validation_v3_output.md", "w", encoding="utf-8") as f:
        f.write(full_output)
        
    print(f"Validation complete! Total time: {generation_time:.2f}s")
    print(f"Generated {total_objects} objects at {ops:.2f} obj/sec")
    print("Results saved to production_validation_v3_report.md and production_validation_v3_output.md")
