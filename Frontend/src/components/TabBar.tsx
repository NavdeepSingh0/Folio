...
export function TabBar({ paneId, tabs, activeTabId, onTabSelect, onTabClose, onTabDrop, onSplitPane, canSplit }: TabBarProps) {
  ...
  return (
    <div 
      className="flex items-center h-10 bg-[#f3f4f6] border-b border-[var(--color-border)] overflow-x-auto select-none"
        ...
        return (
          <div
            ...
            className={`flex items-center min-w-[120px] max-w-[200px] h-full px-3 border-r border-[var(--color-border)] cursor-pointer group transition-colors ${
              isActive ? "bg-white text-blue-600 border-t-2 border-t-blue-600" : "bg-transparent text-gray-600 hover:bg-gray-200 border-t-2 border-t-transparent"
            }`}
          >
            ...
            <button
              ...
              className={`p-0.5 rounded-md hover:bg-gray-300 ml-2 flex-shrink-0 ${isActive ? 'text-gray-500 hover:text-gray-800' : 'text-transparent group-hover:text-gray-500'} transition-colors`}
            >
              <X size={14} />
            </button>
          </div>
