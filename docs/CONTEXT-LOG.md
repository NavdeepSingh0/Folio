...
```
[2026-07-01 12:45] NON-SLICE: Gemini API Integration & Scroll Fixes
What was built: Migrated embeddings_service.py to the unified google-genai SDK using gemini-embedding-001. Configured fallbacks in gemini_service.py to support GEMINI_API_KEY and automatically map gemini-1.5/qwen3 models to gemini-2.5-flash. Fixed flexbox scroll propagation in StudySidebar.tsx.
Key decisions made: Mapped legacy model names inside the API wrappers to avoid editing multiple configuration files and API endpoints, securing immediate application-wide compatibility.
Blockers encountered: The google-genai client internally relies on GOOGLE_API_KEY environment variable, requiring a custom fallback for GEMINI_API_KEY.
Next slice: Cross-Document Intelligence & OCR (Slice 13)
```